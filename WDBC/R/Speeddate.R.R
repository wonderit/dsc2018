...
for(stage in 1:3){
  ...
  if(stage==1){
    ...
    remove.var = c("attractive_o", "sinsere_o", "intelligence_o",
                   "funny_o", "ambitous_o", "shared_interests_o", yvar);
  } else if(stage==3){
    remove.var = c("attractive_o", "sinsere_o", "intelligence_o",
  ...
  write.table(use.xvar, file = sprintf("../Res/stage%02d.txt", stage));
  ...
  for(i in 1:ncol(rmse)){
    ...
    R2["linear regression", i] = min(r2_lm);
